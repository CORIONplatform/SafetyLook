...
namespace MebiusLib
{
    class SJCLDecryptor
    {
        ...
        public SJCLDecryptor(string aJson, string aPW)
        {
            ...
            //}
            // replaced with newtonsoft decoder
            SJCLJSON sjclObj = Newtonsoft.Json.JsonConvert.DeserializeObject<SJCLJSON>(aJson);
            sjclObj.init(aPW);
        ...
        private uint[] sjclDecryptAes()
        {
            ...
            // compute the length of the length ez egy hulladék bazmeg értelmes ember így nem titkosít
            uint olx = (uint)ol;
            ...
            // ez mindig itt hasalt el a tag ellenörzésnél ez lesz a jó pedig csak ebben sok baskodás is van
            // de ha ilyen hulladékkal titkosították mit lehet tenni? :) igen én is itt kaptam agyfaszt mikor ezt meg láttam
        ...
        private class SJCLJSON
        {
            ...
            private byte[] genKeyBytes(string aPW)
            {
                ...
                byte[] saltByte = Encoding.UTF8.GetBytes(this.salt);
                //PBKDF2HMACSHA256 k = new PBKDF2HMACSHA256(pw, saltByte, this.iter);
                //return k.GetBytes(this.ks/8);
                return PBKDF2HMACSHA256Extensions.DeriveKey(aPW, saltByte, this.iter, this.ks);
            }
        }
    }
}
